...
MAP_ROWS=(
  ...
  "inbound-transport|入站 inbound 编辑 edit 用户 user 链接 link 分享 二维码 QR VLESS VMess Trojan Shadowsocks SOCKS HTTP Hysteria2 WireGuard Tunnel TUN RAW XHTTP gRPC WebSocket REALITY TLS 证书 SNI|lib/xray-manager-core.sh|tests/smoke-configs.sh,tests/manager-menu-update.sh|docs/USAGE.md,README.md|inbound,user,share,link,transport,reality,tls,certificate,add_"
  ...
  "ci-tests|CI Actions ShellCheck test smoke 测试失败 workflow|.github/workflows/shellcheck.yml,.github/workflows/publish-r2.yml,tests/smoke-configs.sh|tests/maintainer-map.sh,tests/bootstrap-install.sh,tests/offline-install.sh,tests/cloudflare-update.sh,tests/cloudflare-core-download.sh,tests/config-migration.sh,tests/manager-menu-update.sh,tests/xray-release-delay.sh,tests/geodata-release-delay.sh|CONTRIBUTING.md,docs/MAINTAINER_GUIDE.md|"
)
