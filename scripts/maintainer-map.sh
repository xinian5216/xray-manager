...
MAP_ROWS=(
  "launcher|launcher 启动器 自更新 self-update 菜单更新 版本|xray-manager.sh,install.sh|tests/bootstrap-install.sh,tests/cloudflare-update.sh,tests/manager-menu-update.sh|README.md,docs/PRIVATE_INSTALL.md|update,version,download,install"
  ...
  "inbound-transport|入站 inbound 详情 detail 编号 index 诊断 diagnose 外部 external 编辑 edit 用户 user 链接 link 分享 二维码 QR SS2022 VLESS VMess Trojan Shadowsocks SOCKS HTTP Hysteria2 WireGuard Tunnel TUN RAW XHTTP gRPC WebSocket REALITY TLS 证书 SNI|lib/xray-manager-core.sh|tests/inbound-management.sh,tests/smoke-configs.sh,tests/manager-menu-update.sh|docs/USAGE.md,README.md|inbound,user,share,link,transport,reality,tls,certificate,add_"
  ...
  "xray-geodata|Xray-core core geodata geoip geosite 更新 延迟 release 14天 7天|lib/xray-manager-core.sh,scripts/select-xray-release.sh,scripts/select-geodata-release.sh,XRAY_VERSION,.github/workflows/publish-r2.yml|tests/xray-release-delay.sh,tests/geodata-release-delay.sh,tests/smoke-configs.sh|README.md,CHANGELOG.md|update_xray,update_geodata,release"
  ...
  "ci-tests|CI Actions ShellCheck test smoke 测试失败 workflow|.github/workflows/shellcheck.yml,.github/workflows/publish-r2.yml,tests/smoke-configs.sh|tests/inbound-management.sh,tests/maintainer-map.sh,tests/bootstrap-install.sh,tests/offline-install.sh,tests/cloudflare-update.sh,tests/cloudflare-core-download.sh,tests/config-migration.sh,tests/manager-menu-update.sh,tests/xray-release-delay.sh,tests/geodata-release-delay.sh|CONTRIBUTING.md,docs/MAINTAINER_GUIDE.md|"
)
