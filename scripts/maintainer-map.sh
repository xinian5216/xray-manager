...
MAP_ROWS=(
  "launcher|launcher 启动器 自更新 self-update 菜单更新 版本 降级 downgrade 锁 lock 并发|xray-manager.sh,install.sh,lib/xray-manager-core.sh|tests/bootstrap-install.sh,tests/cloudflare-update.sh,tests/manager-menu-update.sh,tests/version-lock.sh|README.md,docs/PRIVATE_INSTALL.md,docs/USAGE.md|update,version,download,install,lock"
  ...
  "ci-tests|CI Actions ShellCheck test smoke 测试失败 workflow|.github/workflows/shellcheck.yml,.github/workflows/publish-r2.yml,tests/smoke-configs.sh|tests/inbound-management.sh,tests/maintainer-map.sh,tests/bootstrap-install.sh,tests/offline-install.sh,tests/cloudflare-update.sh,tests/cloudflare-core-download.sh,tests/config-migration.sh,tests/backup-restore.sh,tests/version-lock.sh,tests/manager-menu-update.sh,tests/xray-release-delay.sh,tests/geodata-release-delay.sh|CONTRIBUTING.md,docs/MAINTAINER_GUIDE.md|"
)
