...
MAP_ROWS=(
  "launcher|launcher 启动器 自更新 self-update 菜单更新 版本 降级 downgrade 锁 lock 并发 回滚 rollback 原子 current previous 发布|xray-manager.sh,install.sh,offline-install.sh,lib/xray-manager-core.sh|tests/bootstrap-install.sh,tests/cloudflare-update.sh,tests/manager-menu-update.sh,tests/version-lock.sh,tests/atomic-release.sh|README.md,docs/PRIVATE_INSTALL.md,docs/USAGE.md|update,version,download,install,lock,rollback,release"
